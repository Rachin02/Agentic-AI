...
class CompanyKnowledgeBase:
    def __init__(self, collection_name= "company_policies"):
        print("⚙️ Stark AI Knowledge Base...")
        self.embedding = OpenAIEmbeddings(model = "text-embedding-3-large")

        self.vector_store = Chroma(embedding_function= self.embedding, collection_name= collection_name)

        self.llm = ChatOpenAI(model = "gpt-5.4-nano", temperature= 0.1)


    # CRUD operation for RAG

    def add_policy(self, content:str, dept:str, doc_name:str, page_number:int)->str:     ## ADD DATA TO THE VECTOR DATABASE
        """Add a chunk of policy text to the vector database with metadata."""
        doc_id = str(uuid4())
        doc = Document(
    ...
    def update_policy(self, doc_id: str, new_content:str, dept: str, doc_name:str, page_number: int)->str:       ## UPDATE EXISTING DATA
        """Update an existing policy (e.g., when company rules change)"""
        doc = Document(
    ...
    def delete_policy(self, doc_id:str):
        """Remove an outdated policy entirely."""
        self.vector_store.delete(ids = [doc_id])

    ## THE CODE RAG PIPELINE

    def ask_assistant(self, query: str, dept_filter:str = None):
        """EXECUTE RAG"""
        ...
        if dept_filter:
            search_kwargs["filter"] = {"department":dept_filter}
            print(f"   Applying Metadata Filter: Department = {dept_filter}")
       ...
       # 2. AUGMENTATION: Inject the context into the LLM's prompt
        ...
        template = """
        You are STARK companies internal HR and IT assistant. 
...
def main():
    ...
    it_equipment_id = bot.add_policy(
    ...
    while True:
        print("\n" + "="*60)
        print("STARK company KNOWLEDGE PORTAL - Select an action:")
        ...
        if choice in ["1", "2"]:
            ...
            answer, sources = bot.ask_assistant(q, dept_filter=filter_val)
            ...
            bot.update_policy(
                remote_work_id, 
                "STARK is now a remote-first company. Employees may work from anywhere 5 days a week. Office attendance is optional.", 
...
# python3 langchain_chroma/chroma_demo.py
